...
const App: React.FC = () => {
  ...
  const renderStepIndicator = () => (
    <div className="mb-6">
        <p className="text-center text-sm font-semibold text-orange-700">Bước {currentStep} / 3</p>
        <div className="w-full bg-stone-200 rounded-full h-1.5 mt-1">
            <div className="bg-orange-600 h-1.5 rounded-full" style={{ width: `${(currentStep / 3) * 100}%` }}></div>
        </div>
    </div>
  );
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          {!isSubmitted ? (
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              {currentStep === 3 && (
                <div className="space-y-6 animate-fade-in">
                    <FormField label="Bạn có muốn bình luận gì thêm không?">
                        <textarea
                        name="comments"
                        rows={5}
